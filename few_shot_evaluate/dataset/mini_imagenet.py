...
class ImageNet(Dataset):
    def __init__(self, args, partition='train', pretrain=True, is_sample=False, k=4096,
        ...
        with open(os.path.join(self.data_root, self.file_pattern % partition), 'rb') as f:
            ...
            self.imgs = data['data']    # self.imgs is a numpy array with shape: (38400, 84, 84, 3)
            self.labels = data['labels']    # A list with length 38400

            min_labels = min(self.labels)
            self.labels = [a - min_labels for a in self.labels] 
...
class MetaImageNet(ImageNet):
    ...
    def __getitem__(self, item):
        if self.fix_seed:
            np.random.seed(item)
        cls_sampled = np.random.choice(self.classes, self.n_ways, False) # Randomly sample self.n_ways categories.
        ...
        for idx, cls in enumerate(cls_sampled):
            ...
            support_xs_ids_sampled = np.random.choice(range(imgs.shape[0]), self.n_shots, False)    # Randomly sampel self.n_shots image index.
            support_xs.append(imgs[support_xs_ids_sampled])
            support_ys.append([idx] * self.n_shots)
            query_xs_ids = np.setxor1d(np.arange(imgs.shape[0]), support_xs_ids_sampled)    # Avoid sample the images which have been sampled.
            query_xs_ids = np.random.choice(query_xs_ids, self.n_queries, False)    # Sample self.n_queries query images.
