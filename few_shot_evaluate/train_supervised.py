...
def parse_option():
    ...
    parser.add_argument('--model_remark', type = str, default = 'resnet18_scratch', help = 'Describe the model setting.')
    parser.add_argument('--pretrained_model_path', type = str, help = 'The path of saving pretrained model.')
    ...
    opt.tb_folder = os.path.join(opt.tb_path, opt.model_remark)
    if not os.path.isdir(opt.tb_folder):
        os.makedirs(opt.tb_folder)

    opt.save_folder = os.path.join(opt.model_path, opt.model_remark)
...
def main():
    ...
    if opt.pretrained_model_path != None:
        ...
        ckpt = torch.load(opt.pretrained_model_path)
