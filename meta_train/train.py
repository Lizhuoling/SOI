...
from IN_resnet import IN_resnet12, IN_resnet18, IN_resnet50
...
def parse_args():
    parser = argparse.ArgumentParser(description='Surf on the internet using contrastive learning.')
    parser.add_argument('--data_path', type = str, metavar = 'DIR',
                    help='path to dataset')
    parser.add_argument('--checkpoints', type = str, default = './checkpoints', metavar = 'DIR',
                    help = 'The path to save checkpoints.')

    model_names = sorted(name for name in torchvision.models.__dict__
        if name.islower() and not name.startswith("__")
        and callable(torchvision.models.__dict__[name]))
    parser.add_argument('--model_name', type = str, default = 'resnet50', choices = model_names, metavar = 'ARCH',
                    help ='The name of selected model.')
    ...
    parser.add_argument('--dist_url', default = 'tcp://localhost:23455', type = str,
    ...
    parser.add_argument('--data_rate', default = 1, type = float,
                    help = "The usage rate of total data for training.")
...
def main():
    ...
    if os.path.exists(args.checkpoints) is False:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = Build_model(torchvision.models.__dict__[args.model_name], args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp)
    if args.distributed:
        if args.gpu is not None:
    ...
    train_dataset = loader.OfflineLoader(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best = False, filename = 'checkpoint_{:04d}.pth.tar'.format(epoch), args = args)
...
def save_checkpoint(state, is_best, args, filename = 'checkpoint.pth.tar'):
    torch.save(state, os.path.join(args.checkpoints, filename))
    if is_best:
        shutil.copyfile(filename, os.path.join(args.checkpoints, 'model_best.pth.tar'))
