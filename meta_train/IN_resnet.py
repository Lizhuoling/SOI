...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = (self.bn1(out) + self.in1(out)) / 2
        out = self.relu(out)

        out = self.conv2(out)
        out = (self.bn2(out) + self.in2(out)) / 2
        out = self.relu(out)

        out = self.conv3(out)
        out = (self.bn3(out) + self.in3(out)) / 2
        if self.use_se:
            out = self.se(out)
...
def IN_resnet12(keep_prob=1.0, avg_pool=True, **kwargs):
...
def IN_resnet18(keep_prob=1.0, avg_pool=True, **kwargs):
...
def IN_resnet24(keep_prob=1.0, avg_pool=True, **kwargs):
...
def IN_resnet50(keep_prob=1.0, avg_pool=True, **kwargs):
...
def IN_resnet101(keep_prob=1.0, avg_pool=True, **kwargs):
...
def seresnet12(keep_prob=1.0, avg_pool=True, **kwargs):
    """Constructs a ResNet-12 model.
    """
...
def seresnet18(keep_prob=1.0, avg_pool=True, **kwargs):
    """Constructs a ResNet-18 model.
    """
...
def seresnet24(keep_prob=1.0, avg_pool=True, **kwargs):
    """Constructs a ResNet-24 model.
    """
...
def seresnet50(keep_prob=1.0, avg_pool=True, **kwargs):
    """Constructs a ResNet-50 model.
    indeed, only (3 + 4 + 6 + 3) * 3 + 1 = 49 layers
    """
...
def seresnet101(keep_prob=1.0, avg_pool=True, **kwargs):
    """Constructs a ResNet-101 model.
    indeed, only (3 + 4 + 23 + 3) * 3 + 1 = 100 layers
    """
